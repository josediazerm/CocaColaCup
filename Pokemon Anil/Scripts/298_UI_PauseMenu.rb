...
MenuHandlers.add(:pause_menu, :relearn_moves, {
  "name"      => _INTL("Rec Mov"),
  "order"     => 75,
  "condition" => proc { next $player.party_count > 0 },
  "effect"    => proc { |menu|
    ...
    pbFadeOutIn(99999) do
      pbChoosePokemon(1, 3, proc { |p|
        !p.isEgg? && !(p.isShadow? rescue false) && pbHasRelearnableMove?(p)
      })
      if pbGet(1) >= 0
        pokemon = $player.party[pbGet(1)]
        if pokemon
          moves = pbGetRelearnableMoves(pokemon)
          pbRelearnMoveScreen(pokemon) if moves && moves.length > 0
        end
      end
    end
    menu.pbRefresh
...
MenuHandlers.add(:pause_menu, :open_pc, {
  "name"      => _INTL("Abrir PC"),
  "order"     => 76,
  "condition" => proc { next $player.party_count > 0 },
  "effect"    => proc { |menu|
    pbPlayDecisionSE
    if pbInSafari?
      Kernel.pbMessage(_INTL("No puedes usar el PC en la Zona Safari."))
    elsif $game_player.pbHasDependentEvents?
      Kernel.pbMessage(_INTL("No puedes usar el PC cuando estas acompanado."))
    elsif !pbGetMetadata($game_map.map_id, MetadataOutdoor)
      Kernel.pbMessage(_INTL("No puedes usar el PC en interiores."))
    else
      pbFadeOutIn(99999) do
