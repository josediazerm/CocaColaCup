    ...
    loop do
      ...
      if Input.trigger?(Input::USE)
        if $PokemonGlobal.eggs[@index] == nil
          ret = pbConfirmMessage("La incubadora está vacía\\n¿Quieres agregar un Huevo?")
          if ret == true
            if Settings::INCUBATOR_CHOOSE_EGG_FROM_PC
              ...
              if chosen != -1 && $player.party[chosen] != nil
                if !$player.party[chosen].egg?
                  pbMessage("El Pokémon elegido no es un Huevo.")
                else
          ...
          ret = pbConfirmMessage("¿Quieres sacar este Huevo de la incubadora?")
          if ret == true
