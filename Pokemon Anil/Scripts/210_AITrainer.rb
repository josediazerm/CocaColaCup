...
class Battle::AI::AITrainer
  ...
  def set_up_skill
    if @trainer
      ...
      # Modo Radical: IA siempre al máximo para entrenadores
      if $game_switches && $game_switches[MODO_RADICAL] && @skill < 100
        @skill = 100
      end
    elsif $game_switches && $game_switches[MODO_RADICAL]
      # Modo Radical: salvajes también al máximo
      @skill = 100
