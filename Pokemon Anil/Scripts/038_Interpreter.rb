...
class Interpreter
  ...
  #-----------------------------------------------------------------------------
  # Wonder Trade / Intercambio Prodigioso (Don Prodigio)
  # shiny_chance: probabilidad de que el Pokémon recibido sea shiny (0.1 = 10%)
  # infinite: si es true, no activa el self-switch "A" (permite repetir)
  #-----------------------------------------------------------------------------
  def tradeExpert(shiny_chance = 0.1, infinite = false)
    # Elegir un Pokémon del equipo para intercambiar
    chosen = -1
    pbFadeOutIn do
      ...
      screen.pbStartScene(
        $player.party,
        _INTL("¿Qué Pokémon quieres intercambiar?"),
        nil
      )
      loop do
        ...
        if pkmn.egg?
          screen.pbDisplay(_INTL("No puedes intercambiar un Huevo."))
          chosen = -1
        elsif pkmn.shadowPokemon?
          screen.pbDisplay(_INTL("No puedes intercambiar un Pokémon Oscuro."))
          chosen = -1
        elsif $player.party.length <= 1
          screen.pbDisplay(_INTL("No puedes intercambiar tu único Pokémon."))
          chosen = -1
        else
          break
        end
    ...
    # Si cancela, no hacer nada (no activar self-switch)
    return if chosen < 0
    # Confirmar el intercambio
    myPokemon = $player.party[chosen]
    if !pbConfirmMessage(_INTL("¿Quieres intercambiar a {1}?", myPokemon.name))
      return
    end
    # Generar un Pokémon aleatorio del mismo nivel
    level = myPokemon.level
    all_species = []
    GameData::Species.each do |sp|
      next if sp.form != 0
      all_species.push(sp.id)
    end
    random_species = all_species[rand(all_species.length)]
    yourPokemon = Pokemon.new(random_species, level)
    traderName = get_self.name rescue "Don Prodigio"
    yourPokemon.owner = Pokemon::Owner.new_foreign(traderName, rand(2))
    yourPokemon.obtain_method = 2   # traded
    yourPokemon.reset_moves
    yourPokemon.record_first_moves
    # Aplicar probabilidad de shiny
    if rand < shiny_chance
      yourPokemon.shiny = true
    end
    # Realizar la animación de intercambio
    $stats.trade_count += 1
    pbFadeOutInWithMusic do
      evo = PokemonTrade_Scene.new
      evo.pbStartScreen(myPokemon, yourPokemon, $player.name, traderName)
      evo.pbTrade
      evo.pbEndScreen
    end
    $player.party[chosen] = yourPokemon
    # Activar self-switch "A" solo si no es infinito
    if !infinite
      setTempSwitchOn("A")
    end
  end
